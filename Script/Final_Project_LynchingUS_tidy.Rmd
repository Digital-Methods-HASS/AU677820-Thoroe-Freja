...
# An introduction to the used datasets, and final graphs
...
```
However, when entering my own data, it did not seem to work. There was a fault in aesthetics. That I couldn't figure out to solve.
...
head(Lynchings_sorted_by_county)
```
...
rename(mapdata, county=subregion)
```
...
head(Lynching_counties_merge)
```
It has after so many tutorials, hours, sweat and tears finally merged together my lynching dataset with county coordinates.

I first create a new spacialpoints dataframe. Not sure why I did this step.
```{r}
Lynching_counties_merge_SP <- SpatialPointsDataFrame(Lynching_counties_merge[,c(19,20)],Lynching_counties_merge[,-c(19,20)])
```


```{r}
Lynching_Rigby_map <- leaflet() %>%
  addTiles() %>%
  addMarkers(data = Lynching_counties_merge, lng = ~long, lat = ~lat, popup = ~victim)
Lynching_Rigby_map
```
